...
const AddMaintenance = () => {
  ...
  const { loading } = useSelector((s) => s.maintenance);
  ...
  const onSubmit = async (data) => {
    const res = await dispatch(createMaintenance({
      ...data,
    ...
    <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-900">Add Maintenance</h1>
          <p className="text-sm text-gray-400">Create a monthly maintenance charge for a resident</p>
        </div>
              ...
              >
                ...
                {residents.map((r) => (
                  <option key={r._id} value={r._id}>
                    {r.flatNumber || r.flat || ""} {r.name ? `— ${r.name}` : ""}
                  </option>
                ))}
              </select>
