        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                [...Array(5)].map((_, i) => <tr key={i} className="animate-pulse"><td colSpan="6" className="p-8 bg-gray-50/50"></td></tr>)
              ) : filtered.length > 0 ? (
                filtered.map((notice) => (
                  <tr key={notice._id} className={`hover:bg-blue-50/30 transition-colors ${!notice.isActive ? "bg-gray-50/50" : ""}`}>
                    <td className="px-6 py-4">
                      <div className="max-w-xs">
                        <div className={`font-bold ${notice.isActive ? "text-gray-900" : "text-gray-400"}`}>{notice.title}</div>
                        <div className="text-[11px] text-gray-400 line-clamp-1">{notice.content}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
