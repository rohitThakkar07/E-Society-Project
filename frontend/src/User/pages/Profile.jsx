...
import {
  User, Phone, FileText, Briefcase, MapPin,
...
const ProfilePage = () => {
  ...
  const { singleResident: data, loading } = useSelector((state) => state.resident);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 flex flex-col gap-6">
            {/* TAB NAVIGATION */}
            <div className="bg-white p-2 rounded-[24px] card-shadow border border-slate-100 flex gap-1 overflow-x-auto no-scrollbar">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`px-6 py-3 rounded-[18px] flex items-center gap-2 text-sm font-bold transition-all whitespace-nowrap ${activeTab === tab.id
